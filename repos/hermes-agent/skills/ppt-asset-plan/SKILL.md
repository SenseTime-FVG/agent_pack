...
- 如果 `asset_requirements` 写得过轻，但 `visual_requirements` 或页面语义明显指向真实图片，应补出对应的 `real-photo` 槽位，并在 `reason` 中注明修正原因。
- 如果 `asset_requirements` 写得过轻
- 但 `visual_requirements` 或页面语义明显指向真实图片
- 补出对应的 `real-photo` 槽位
...
- 不要让三张人物卡只规划一个图片槽位。
- 不要让双图展示页只落一个 `selected_image`。
- 不要把人物、产品、场景、活动现场、作品样张等应由真实图片承载的内容误判为 `svg-illustration`。
- 不要把人物、产品、场景、活动现场误判成 `svg-illustration`。
...
- **开始反馈**：说明有多少页面、多少槽位需要补图，以及将会生成哪些工件。
- **进行中**：搜图/下载明显耗时时，补 1 条进度更新，说明已完成和未处理的槽位数。
- **完成反馈**：总结成功落地的本地图片数量、仍 `unresolved` 的槽位数量和 `下一步`。
