...
使用本 skill 时，先自动检测是否已配置 API Token，再决定渠道：
...
# 检查配置文件（如环境变量为空）
cat ~/.mineru/config.yaml 2>/dev/null | grep -q 'token' && echo "configured" || echo "not_configured"
```

### 检测结果处理

- **如果检测到 Token 已配置**：默认使用 `extract`（Token/API 完整模式），无需询问用户。直接告知用户正在使用 Token 模式。
- **如果未检测到 Token**：调用 `ask_user` 询问用户：

> 未检测到 MinerU API Token，你想走哪种渠道？
> 1. 官方免费快速模式：无需 token，但有 10MB / 20 页限制，且不含表格识别
> 2. Token/API 完整模式：需要先创建 token（https://mineru.net/apiManage/token），支持更大文件、表格/公式识别和多格式导出
...
当未检测到 Token 时，不要替用户默认选择，必须询问用户。

## Core workflow

1. **auto_detect_or_ask**: 先检测环境变量 `MINERU_TOKEN` 或 `~/.mineru/config.yaml` 是否已配置 Token。若已配置则默认使用 `extract`；若未配置则 ask_user 选择免费或 Token 模式。
2. **Start fast** (no token): `mineru-open-api flash-extract <file>` for quick Markdown conversion
3. **Need more?** Create token at https://mineru.net/apiManage/token, run `mineru-open-api auth`, then use `mineru-open-api extract` for tables, formulas, OCR, multi-format, and batch
4. **Check results**: output goes to stdout (default) or `-o` directory

## Authentication

Only required for `extract` . Not needed for `flash-extract`.
...
```
~/<workspace>/mineru_paerser/<name>_<hash>/
```
**Naming rules:**
