...
4. **发现报告格式**：必要时读取 `report-format-discovery`，识别报告类型、可信结构来源、必备章节、强制元素和风格约束，再压缩进 `plan.json.report_shape`。
...
可以直接从 `request.md` 推断 `genre`、`domain`、`audience`、`region`；若这些信息不足以稳健决定结构，再读取 `report-format-discovery`。
...
- `report_shape.format_basis`：说明报告结构依据来自哪里。若调用过 `report-format-discovery`，至少保留 1 个依据；若未调用，也要写明是“用户指定结构”或“通用领域惯例”。
